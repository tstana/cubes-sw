...
#ifndef MSP_ERROR_THRESHOLD
#define MSP_ERROR_THRESHOLD 5
#endif

// Increase buffer size if MTU is larger than 4096
static const unsigned long REQUEST_BUFFER_SIZE = EXP_MTU > 4096 ? EXP_MTU : 4096;
...
void invoke_request(msp_link_t *lnk, unsigned char opcode, PrintStyle pstyle)
{
	...
	if (request_buffer_overflow) {
		Serial.println(F("Buffer Overflow (Received more than 4096 bytes of data.)"));
